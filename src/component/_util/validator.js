const rules = {
  ...
  pwd: {
    validate: function (v, n) {
      return /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,15}$/.test(v);
    },
    getMsg: function (l, n) {
      return '密码为8-15位数字和字母组合,不含特殊字符';
    }
  },
  //最大字符长度
  max_length: {
...
class Validator {
  ...
  validate = (v) => {
    ...
    let result = this.rule?.validate?.(v, this.params);
    if (!result) {
      return {
        key: this.key,
        msg: typeof this.rule?.getMsg === 'function' ? this.rule.getMsg(this.label, this.params) : this.rule.getMsg,
      };
    }
    return null;
  };
...
const createValidator = function (rulesText, label = '') {
  ...
  list = list.map(item => {
    ...
    switch (length) {
      ...
      case 5:
        console.warn('[pgyos]: Rules accept up to two parameters！');
        return null;
      default:
        console.warn('[pgyos]: No matching rules found！');
        return null;
    }
