          ...
          >
            <template v-slot:expand-row="{row}">
              <div class="p-15">
                ...
                <div class="row">
                  <div class="col-3">
                    所属门店：{{ row.store && row.store.title || '-' }}
                  </div>
                </div>
              </div>
            </template>
            <pg-column title="提现单号" width="120px" prop="code">
              <template v-slot="{row}">
                <a class="text-dark" @click="handleDetailItem(row)">{{ row.code }}</a>
              </template>
            </pg-column>
            <pg-column title="申请人/手机号" width="140px">
              <template v-slot="{row}">
                {{ row.realname || '-'}}/{{row.acc_phone || '-'}}
              </template> 
            </pg-column>
            <pg-column title="转账银行" prop="bank_acc" width="120px"></pg-column>
            <pg-column title="银行卡号" width="150px">
              <template v-slot="{row}">
                {{ row.acc_card_no || '-' }}
            ...
            <pg-column title="提现申请金额" width="70px" text-align="center">
              <template v-slot="{row}">
                {{ !!row.amount ? '¥' : '' }}{{ Handle.returnPrice(row.amount) }}
              </template>
            </pg-column>
            <pg-column title="手续费" width="70px" text-align="center">
              <template v-slot="{row}">
                {{ !!row.amount ? '¥' : '' }}{{ Handle.returnPrice(row.fee) }}
              </template>
            </pg-column>
            <pg-column title="实际到账金额" width="70px" text-align="center">
              <template v-slot="{row}">
                {{ !!row.amount ? '¥' : '' }}{{ Handle.returnPrice(row.real_amount) }}
              </template>
            </pg-column>
            <pg-column title="状态" width="80px" text-align="center">
              <template v-slot="{row}">
                <div class="d-flex justify-content-center align-items-center overflow-ellipsis">
                  <span :class="`status-dot mr-5 bg-${Constant.STORE_WITHDRAW_STATUS('color')[row.status]}`"></span>
                  {{ Constant.STORE_WITHDRAW_STATUS('enum')[row.status] || row.status || '-' }}
                </div>
              </template>
            </pg-column>
