<template>
  <div class="p-20">
    <div class="d-flex">
      <pg-button class="ml-auto" color="primary" v-if="app.auth.isAdmin || app.auth.MerchantGradeAdd" @click="handleAddItem">新增</pg-button>
    </div>
    <div class="card mt-10">
      <div class="card-body">
        <pg-table
          ...
          borderless
        >
          <pg-column prop="level" title="编号" width="100px" text-align="center">
            <template v-slot="{row}">
              {{ row.code }}
            </template>
          </pg-column>
          ...
          <pg-column title="操作" width="90px">
            <template v-slot="{row}">
              <a
                ...
                v-if="(app.auth.isAdmin || app.auth.MerchantGradeEdit)"
                @click="handleModifyItem(row)"
              >修改</a>
              <pg-confirm
                v-if="(app.auth.isAdmin || app.auth.MerchantGradeDelete)"
                help-text="确认删除该商户等级"
    ...
    <pg-dialog v-model="dialog.visible" :title="`${dialog.type === 'add' ? '新增' : '修改'}商户等级`">
      <grade-edit
        v-if="dialog.visible"
        :type="dialog.type"
        :item="dialog.item"
...
export default {
  ...
  methods: {
    ...
    gradeQuery() {
      Http.get(Api.merchantPropertyGradeList)
        .then(res => {
    ...
    handleDeleteItem(item) {
      Http.post(Api.merchantPropertyGradeDelete, { id: item.id })
        .then(() => {
          this.$toast({ type: 'success', message: '商户等级删除成功' });
          this.gradeQuery();
        });
    },
